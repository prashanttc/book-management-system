  ...
  return (
    <div>
     ...
     <div className='sidebar flex flex-col overflow-hidden w-[20%] min-h-[100vh] z-10 fixed bg-gray-800 pt-10'>
      ...
      <div className='w-[80%] absolute right-0 text-black'> <div className='h-20 flex text-black   text-4xl font-semibold drop-shadow-2xl bg-white items-center justify-center w-full '>Delete books</div> <Outlet/>
         ...
         {books.map((book) => (
           <div className="book flex flex-col items-center" key={book.id}>
            ...
            <div className="flex gap-5 text-white mt-5">
               <button onClick={()=>handleDelete(book.id)} className="rounded-3xl px-[40px] py-[15px] border-2 bg-red-600/80  hover:bg-red-600"> <Link to="/Remove">Delete</Link></button>
            </div>
           </div>
         ))}
       </div></div>
